...
// Juz start positions now come from utils/hifzTargets — the single
// copy shared with Round Mode (this file used to carry a duplicate).
...
export function HifzLogEntry({
  ...
  useEffect(() => {
    ...
    const repeat = missed || isRepeatRating(quality);
  ...
  const handleSubmit = async (after: "close" | "kind" | "student" = "close") => {
    ...
    if (revDay) {
      ...
      const repeatRev = quality === "" || isRepeatRating(quality);
      ...
      const repeatRev = isRepeatRating(quality);
